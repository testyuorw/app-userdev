  ...
  <div class="pd-select-item">
    ...
    <div class="pd-select-list">
      <ul class="pd-select-ul" ref="list">
        <li class="pd-select-list-item" v-for="el,index in renderData " :class="{'hidden':setHidden(el.index)}" :key="index">{{el.value.name}}</li>
      </ul>
    </div>
    <ul class="pd-select-wheel" ref="wheel">
      <li class="pd-select-wheel-item" :class="{'hidden':setHidden(el.index)}" :style="setWheelItemDeg(el.index)" :index="el.index" v-for="el,index in renderData " :key="index">{{el.value.name}}</li>
    </ul>
  </div>
</template>
<script>
  ...
  export default{
    ...
    mounted () {
      ...
      var self = this;
      setTimeout(function () {
        let index = self.listData.indexOf(self.value);
        if (index === -1) {
          console.warn('当前初始值不存在，请检查后listData范围！！');
          self.setListTransform.apply(self);
          self.getPickValue.call(self,0);
        } else {
          let move = index * 34
          /* 因为往上滑动所以是负 */
          self.setStyle.call(self,-move);
          self.setListTransform.call(self,-move, -move)
        }
      },0)
    },
    methods: {
      ...
      getSpinData (index) {
        ...
        try{
          ...
          result = this.listData[index >= 0 ? index : index + this.listData.length];
        }catch(e){
      ...
      getPickValue (move) {
        ...
        try{
          index = index % this.listData.length
          index = index >= 0 ? index : index + this.listData.length
        }catch(e){
